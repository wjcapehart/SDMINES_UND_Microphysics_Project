...
C     BoxMG solver (BMG2_SymStd_SOLVE_boxmg).  It demonstrates a modular
C     setup of a discretized PDE and the BoxMG solve.  It uses the 
C     subroutine BMG2_SymStd_SETUP_PtrWork to compute the dimensions
C     of the required workspace and the necessary pointers into that 
C     workspace. In addition, it uses BMG2_SymStd_SETUP_ProcGrid_file
...
C
C     BMG_iPARMS(id_BMG2_MAX_ITERS)   = 1
...
C     To test NOG = 1 
C     
C     BMG_iPARMS(id_BMG2_MIN_NOG)     = 1
C     BMG_iPARMS(id_BMG2_CG_MIN_DIM)  = NLx
...
C ------------------------------
     ...
     &          )
      ...
      IF ( MPI_MyProc.EQ.0 ) THEN
         IF (
     &      BMG_iPARMS(id_BMG2_STOP_TEST).EQ.BMG_STOP_REL_RES_L2 ) THEN 
            WRITE(*,1180) 'Final Relative Residual (l2-norm) = ', TOL
         ELSEIF ( 
     &      BMG_iPARMS(id_BMG2_STOP_TEST).EQ.BMG_STOP_ABS_RES_L2 ) THEN
...
C ===========================================================================
      ...
      BMG_rPARMS(id_BMG2_STOP_TOL) = TOL_SAVE
